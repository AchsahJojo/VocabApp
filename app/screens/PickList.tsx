...
export default function PickList({ route }: PickListProps) {
  ...
  const loadVocabLists = async () => {
    try {
      ...
      console.log("🌐 URL:", `${API_BASE_URL}/api/vocab/lists/${userID}/exclude-history`);
      ...
      console.error("=".repeat(50));
      Alert.alert("Error", "Failed to load vocab lists. Check console for details.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <SafeAreaView style={styles.container}>
        {loading ? (
          <View style={{ marginTop: 20 }}>
            ...
            <Text style={{ textAlign: 'center', marginTop: 10 }}>Loading lists...</Text>
          </View>
        ) : vocabLists.length === 0 ? (
          <View style={{ marginTop: 20, padding: 20 }}>
            ...
            <TouchableOpacity 
              style={[styles.item, { backgroundColor: '#4CAF50', marginTop: 20 }]}
              onPress={() => {
                console.log("➕ Create list button pressed");
                (navigation as any).navigate("ListCreation", { userID });
              }}
            >
              <Text style={[styles.listName, { color: 'white' }]}>+ Create New List</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <>
